...
def main(args):
    print("=" * 60)
    print(f"🚀 QUAD ASPECT ENHANCED BERT TRAINING (ablation={args.ablation_mode}, offline)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("📌 OUTER 10-FOLD (SINGLE MODEL, NO TUNE)")
    print("=" * 60)
    ...
    for fold_id, (tr_idx, te_idx) in enumerate(folds, start=1):
        ...
        # ========== (3) 用 full outer-train 重训 backbone，再在 outer-test 上测 ==========
        full_train_samples = [samples[i] for i in tr_idx]
        test_probs_list = []

        for model_cfg in ENSEMBLE_MODELS:
            backbone = model_cfg["name"]
            print(f"\n🔥 [Outer {fold_id}] Train backbone on FULL outer-train: {backbone}")
        ...
        test_preds = np.argmax(test_probs_list[0], axis=1)  # 👈 Single model prediction
