...
def main(args):
    ...
    for fold_id, (tr_idx, te_idx) in enumerate(folds, start=1):
        ...
        print(f"\n🎯 [Outer {fold_id}] single model, no ensemble, no weight tuning")
        ...
        test_preds = np.argmax(test_probs_list[0], axis=1)
        fold_acc = accuracy_score(test_labels, test_preds)
        fold_f1 = f1_score(test_labels, test_preds, average="macro")
        for idx, (pred, true_lbl) in enumerate(zip(test_preds, test_labels)):
            if pred != true_lbl:
                all_misclassified_samples.append({
                    "fold": fold_id,
                    "text": test_outer_samples[idx]["text"],
    ...
    clean_memory()
    error_output_path = PROJECT_ROOT / "misclassified_samples_ablation.json"
