...
class QuadAspectEnhancedBertModel(PreTrainedModel):
    ...
    def forward(
        ...
        # 用非交互的文本和四元池化结果的平均值代替
        shared_features = (text_pooled + quad_pooled) / 2.0
        fused = self.fusion(text_pooled, quad_pooled, shared_features)
...
def train_asqp_model(args, train_data, eval_data, tokenizer, method_id, result_id, contrib_id, general_id, run_tag: str = ""):
    ...
    results_dir = PROJECT_ROOT / "results" / args.model_name / tag
    logs_dir = PROJECT_ROOT / "logs" / args.model_name / tag
...
def main(args):
    ...
    merged_path = (PROJECT_ROOT / args.merged_json).resolve() if not Path(args.merged_json).is_absolute() else Path(
        args.merged_json)
    ...
    print(
        f"📊 [INFO] Loaded total={len(samples)} | neutral={dist.get('neutral', 0)} pos={dist.get('positive', 0)} neg={dist.get('negative', 0)}")

    # 2) Build OUTER 10-fold splits
    # -------------------------
    label_map = {"neutral": 0, "positive": 1, "negative": 2}
    id2label = {v: k for k, v in label_map.items()}  # 👈 新增：用于将数字ID转回文本标签
    ...
    print(
        f"   ALL: Neutral={dist.get('neutral', 0)} Positive={dist.get('positive', 0)} Negative={dist.get('negative', 0)}")
    ...
    all_misclassified_samples = []  # 👈 新增：用于存储所有预测错误的样本
    ...
    for fold_id, (tr_idx, te_idx) in enumerate(folds, start=1):
        ...
        # 👈 新增：遍历预测结果，找出预测错误的样本并保存
        for i, (pred, true_lbl) in enumerate(zip(test_preds, test_labels)):
            if pred != true_lbl:
                all_misclassified_samples.append({
                    "fold": fold_id,
                    "text": test_outer_samples[i]["text"],
                    "true_sentiment": id2label[true_lbl],
    ...
    # 保存错误样本到 JSON
    # -------------------------
    error_output_path = PROJECT_ROOT / "misclassified_samples.json"
    with open(error_output_path, "w", encoding="utf-8") as f:
        json.dump(all_misclassified_samples, f, ensure_ascii=False, indent=4)
    print(f"\n📁 [INFO] 成功保存 {len(all_misclassified_samples)} 个预测错误的样本至: {error_output_path}")
